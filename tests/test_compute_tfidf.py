...
class TestTFIDFEdgeCases:
    ...
    def test_empty_document(self) -> None:
        ...
        # Empty document causes division by zero with TERM_FREQUENCY
        # Use BINARY weighting instead which handles empty documents gracefully
        score = compute_tfidf("hello", "", corpus, WeightingSchemes.BINARY)
        assert score == 0

    def test_single_document_corpus(self) -> None:
        """Test TF-IDF with single document corpus."""
        corpus = ["hello world"]
        score = compute_tfidf("hello", corpus[0], corpus, WeightingSchemes.TERM_FREQUENCY)
        # Should not raise errors and should return a valid score
        assert score >= 0

    def test_term_not_in_document(self) -> None:
        """Test TF-IDF when term doesn't appear in the document."""
        corpus = ["hello world", "foo bar"]
        score = compute_tfidf("hello", "foo bar", corpus, WeightingSchemes.TERM_FREQUENCY)
        assert score == 0

    def test_term_not_in_corpus(self) -> None:
        """Test TF-IDF when term doesn't appear anywhere in corpus."""
        corpus = ["hello world", "foo bar"]
        score = compute_tfidf("missing", "missing word", corpus, WeightingSchemes.TERM_FREQUENCY)
        # Should return a positive score since the term appears in the document
        # and has high IDF (doesn't appear in other docs)
        assert score > 0

    def test_case_insensitivity(self) -> None:
        """Test that TF-IDF is case-insensitive."""
        corpus = ["The Cat", "the dog", "THE BIRD"]
        score_lower = compute_tfidf("cat", corpus[0], corpus, WeightingSchemes.TERM_FREQUENCY)
        score_upper = compute_tfidf("CAT", corpus[0], corpus, WeightingSchemes.TERM_FREQUENCY)
        score_mixed = compute_tfidf("CaT", corpus[0], corpus, WeightingSchemes.TERM_FREQUENCY)

        assert score_lower == score_upper == score_mixed


class TestDifferentWeightingSchemes:
    """Test TF-IDF with different weighting schemes."""
    ...
    def test_binary_weighting(self, corpus: list[str]) -> None:
        """Test TF-IDF with BINARY weighting scheme."""
        score = compute_tfidf("cat", corpus[0], corpus, WeightingSchemes.BINARY)
        # With BINARY, TF is either 0 or 1
        assert score > 0

    def test_raw_count_weighting(self, corpus: list[str]) -> None:
        """Test TF-IDF with RAW_COUNT weighting scheme."""
        score = compute_tfidf("the", corpus[0], corpus, WeightingSchemes.RAW_COUNT)
        # With RAW_COUNT, TF = count of term
        # "the" appears 2 times in D1
        assert score >= 0

    def test_log_normalisation_weighting(self, corpus: list[str]) -> None:
        """Test TF-IDF with LOG_NORMALISATION weighting scheme."""
        score = compute_tfidf("cat", corpus[0], corpus, WeightingSchemes.LOG_NORMALISATION)
        # With LOG_NORMALISATION, TF = log(1 + count)
        assert score > 0

    def test_double_normalisation_k_weighting(self, corpus: list[str]) -> None:
        """Test TF-IDF with DOUBLE_NORMALISATION_K weighting scheme."""
        score = compute_tfidf("cat", corpus[0], corpus, WeightingSchemes.DOUBLE_NORMALISATION_K)
        # With DOUBLE_NORMALISATION_K, TF = k + (1-k) * count / max_count
        assert score > 0

    def test_term_frequency_matches_example(self, corpus: list[str]) -> None:
        """Test that TERM_FREQUENCY weighting produces expected results."""
        # Using the example from the documentation
        score_cat_d1 = compute_tfidf("cat", corpus[0], corpus, WeightingSchemes.TERM_FREQUENCY)
        score_cat_d2 = compute_tfidf("cat", corpus[1], corpus, WeightingSchemes.TERM_FREQUENCY)

        # D1 has 6 words, D2 has 7 words, both have "cat" once
        # So TF(cat, D1) = 1/6 > TF(cat, D2) = 1/7
        # With same IDF, score_d1 should be > score_d2
        assert score_cat_d1 > score_cat_d2
